...
JSON形式のみで出力してください（\`\`\`json などの装飾は禁止）：
{
...
お題: "${topic}"
これまでの議論ログ:
${history.map((h: any) => `${h.speaker}: ${h.content}`).join('\n')}

次に発言するAI（冷笑派 または 論理派）の返信を生成してください。
返信相手の矛盾を突きつつ、短くレスバしてください。
...
Step 3: app/page.tsx の丸ごと更新
画面のタブに「VS AI特訓」「反論参謀」「AI vs AI 観戦」を追加し、審判・画像読み込み・詭弁検知をすべて統合しました！

以下を app/page.tsx に丸ごと上書き保存してください。
...
export default function Home() {
  const [mode, setMode] = useState<'chat' | 'assist' | 'watch'>('chat');
      ...
      <div className="w-full max-w-[600px] border-x border-gray-800 flex flex-col">
        ...
        <header className="sticky top-0 bg-black/80 backdrop-blur-sm border-b border-gray-800 z-10">
          ...
          <div className="flex w-full border-b border-gray-800">
            {[
              { id: 'chat', label: 'VS AI 特訓' },
              { id: 'assist', label: '反論参謀' },
              { id: 'watch', label: 'AI観戦TV' },
            ].map((tab) => (
              <button key={tab.id} onClick={() => setMode(tab.id as any)} className="flex-1 hover:bg-gray-900 transition">
                <div className={`py-3 text-xs font-bold relative ${mode === tab.id ? 'text-white' : 'text-gray-500'}`}>
                  {tab.label}
                  {mode === tab.id && <div className="absolute bottom-0 left-0 right-0 h-1 bg-blue-500 rounded-full" />}
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="flex-1 flex flex-col">
            <div className="p-4 border-b border-gray-800 space-y-3">
              ...
              <div className="flex gap-2 items-center">
                <button onClick={handleGenerateTopic} disabled={topicLoading} className="bg-blue-500 text-xs px-3 py-1.5 rounded-full font-bold">
                  🎲 お題自動生成
                </button>
                <button onClick={handleJudge} disabled={judgeLoading} className="bg-purple-600 text-xs px-3 py-1.5 rounded-full font-bold">
                  ⚖️ 審判に採点してもらう
                </button>
              </div>
              ...
              {judgeResult && (
                <div className="p-3 bg-purple-950/40 border border-purple-800 rounded-lg text-xs space-y-1">
                  ...
                  {judgeResult.fallacies?.length > 0 && (
            ...
            <form onSubmit={handleSendMessage} className="p-3 border-t border-gray-800 flex gap-2">
              ...
              />
              <button type="submit" disabled={loading} className="bg-blue-500 text-xs px-5 py-2 rounded-full font-bold">
                送信
              </button>
            </form>
          </div>
        ...
        {mode === 'assist' && (
          <div className="flex-1 p-4 space-y-4 overflow-y-auto">
            ...
            <form onSubmit={handleGenerateAssist} className="space-y-3">
              ...
              />
              <button type="submit" disabled={assistLoading} className="w-full bg-blue-500 py-2.5 rounded-full text-xs font-bold">
        ...
        {mode === 'watch' && (
          <div className="flex-1 flex flex-col p-4 space-y-4">
            <div className="flex gap-2">
              ...
              />
              <button onClick={handleNextWatchStep} disabled={watching} className="bg-red-600 px-4 py-1.5 rounded-full text-xs font-bold">
